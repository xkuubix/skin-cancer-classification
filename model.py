...
class FixCapsNet(nn.Module):
    ...
    def loss(self, img_input, target, size_average=True):
        ...
        device = img_input.get_device()
        zero = Variable(torch.zeros(1, dtype=torch.float)).to(device)
        m_plus = torch.tensor(0.9, dtype=torch.float).to(device)
        m_minus = torch.tensor(0.1, dtype=torch.float).to(device)
        epsilon = torch.tensor(1e-7, dtype=torch.float).to(device)
        max_l = torch.abs(torch.max(m_plus - v_mag, epsilon).view(batch_size, -1))**2
        max_r = torch.abs(torch.max(v_mag - m_minus, epsilon).view(batch_size, -1))**2
        # This is equation 4 from the paper.
        loss_lambda = torch.tensor(0.5, dtype=torch.float).to(device)
        T_c = target
...
def make_features(cfg: list,f_c,out_c=None,g=1,step=2):
    ...
    for v in cfg:
        if v == "M":
            ...
            layers += [nn.Conv2d(f_channels, v, 31,stride=step,groups=g)]
            f_channels = v
